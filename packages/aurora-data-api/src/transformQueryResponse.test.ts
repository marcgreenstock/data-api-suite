...
test('stringValue', () => {
  const result = transformQueryResponse({
    ...
    records: [[{ stringValue: 'hello world' }]],
  })
  expect(result).toMatchObject({
    metadata: {
...
test('unknown value', () => {
  const result = transformQueryResponse({
    ...
    records: [[{ weirdValue: 'i am a blob' }]],
  } as any)
  expect(result).toMatchObject({
    metadata: {
      example: { name: 'example' },
...
test('missing name in metadata', () => {
  const result = transformQueryResponse({
    ...
    records: [[{ stringValue: 'hello world' }, { longValue: 7 }]],
  })
  expect(result).toMatchObject({
    metadata: {
      0: { typeName: 'varchar' },
