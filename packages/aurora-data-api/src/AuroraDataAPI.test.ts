...
describe('new AuroraDataAPI', () => {
  ...
  it('sets the transformOptions', () => {
    expect(client.transformOptions).toMatchObject({
      valueTransformer: config.valueTransformer,
    })
  })
  ...
  const executeStatementResponse: RDSDataService.ExecuteStatementResponse = {
    records: [
      [
        { longValue: 1 },
        { stringValue: 'marc' },
        { stringValue: '2019-12-25 19:14:33' },
      ],
      [
        { longValue: 2 },
        { stringValue: 'felix' },
        { stringValue: '2020-01-13 15:21:07' },
      ],
    ],
    columnMetadata: [
      {
...
describe('AuroraDataAPI#batchQuery', () => {
  ...
  it('calls RDSDataService#executeBatchStatement and returns the result', async () => {
    const { database, resourceArn, schema, secretArn } = requestConfig
    const sql =
      'INSERT INTO users (name, interests::varchar[]) VALUES (:name, :interests)'
