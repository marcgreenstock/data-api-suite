...
import { Compiler, CompilerBase } from './Compiler'

export class TypeScriptCompiler extends CompilerBase implements Compiler {
  public async compile(): Promise<string[]> {
    ...
    const files = (await fs.readdir(this.migrationsPath)).map((fileName) =>
